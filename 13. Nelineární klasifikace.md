...
- Je dobré zrychlit konvergenci k minimu:
  - Momentum SGD
    $$
    \boldsymbol{\theta}_{t+1} = \boldsymbol{\theta}_t - \gamma \nabla L(\boldsymbol{\theta}_t) + \alpha \boldsymbol{v}_t
    $$
  - Adam (Adaptive Momentum)
    ...
    $$
      \begin{align*}
      \text{momentum akumulace:} \quad & \boldsymbol{v}_{t+1} := \alpha \cdot \boldsymbol{v}_t + (1-\alpha) \cdot \nabla L(\boldsymbol{\theta}_t) \\
      \text{rmsprop akumulace:} \quad & \boldsymbol{u}_{t+1} := \beta \cdot \boldsymbol{u}_t + (1-\beta) \cdot \nabla L(\boldsymbol{\theta}_t)^2 \\
      \\
      \text{korekce:} \quad & \hat{\boldsymbol{v}}_{t+1} := \frac{\boldsymbol{v}_{t+1}}{1 - \alpha^t} \\
      & \hat{\boldsymbol{u}}_{t+1} := \frac{\boldsymbol{u}_{t+1}}{1 - \beta^t} \\
      \\
      \text{Adam update:} \quad & \boldsymbol{\theta}_{t+1} := \boldsymbol{\theta}_t - \gamma \cdot \frac{\hat{\boldsymbol{v}}_{t+1}}{\sqrt{\hat{\boldsymbol{u}}_{t+1}} + \epsilon},
      \end{align*}
    $$
